...
if FindU ~= -1 and ListBox.GetItemData("unknowns", FindU) == "U" and ListBox.FindItem("Words", -1, LB_BYTEXT, "«‰—éÌ Å «‰”Ì·") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "U") ~= -1 then
	if V_Electric ~= nil and C_Condenser ~= nil then
		Binesh.CreateHTMLFile("U = <span>1/2</span>*C*V<sup>2</sup>")
...
elseif FindC_Condenser ~= -1 and ListBox.GetItemData("unknowns", FindC_Condenser) == "C" and ListBox.FindItem("Words", -1, LB_BYTEXT, "Œ«“‰") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "C") ~= -1 then
	if V_Electric ~= nil and U ~= nil then
		Binesh.CreateHTMLFile("U = <span>1/2</span>*C*V<sup>2</sup>", "C = <span>2*U/V<sup>2</sup></span>")
...
elseif FindV_Electric ~= -1 and ListBox.GetItemData("unknowns", FindV_Electric) == "V" and ListBox.FindItem("Words", -1, LB_BYTEXT, "Ê· «é") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "V") ~= -1 then
	if U ~= nil and C_Condenser ~= nil then
		Binesh.CreateHTMLFile("U = 1/2*C*V<sup>2</sup>", "V = <span>Sqrt(2*U/C)</span>")
