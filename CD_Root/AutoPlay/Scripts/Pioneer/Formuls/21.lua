	...
	if isLens == true then
		if f == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "⁄œ”Ì ›«’·Â ò«‰Ê‰") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "f") ~= -1) then
			if q_Lens ~= nil and p_Lens ~= nil then
				...
				Binesh.AddToHTML("f = <span>"..Binesh.Convert_e(q_Lens).."*"..Binesh.Convert_e(p_Lens).."/"..Binesh.Convert_e(q_Lens).."+"..Binesh.Convert_e(p_Lens).."</span> &#8658; f = "..Binesh.Convert_e(f).."m")
		...
		if q_Lens == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "›«’·Â ”«ÌÂ") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "q") ~= -1) then
			if p_Lens ~= nil and f ~= nil then
				...
				Binesh.AddToHTML("q = <span>"..Binesh.Convert_e(f).."*"..Binesh.Convert_e(p_Lens).."/"..Binesh.Convert_e(f).."-"..Binesh.Convert_e(p_Lens).."</span> &#8658; q = "..Binesh.Convert_e(q_Lens).."m")
		...
		if p_Lens == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "›«’·Â Ã”„") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "p") ~= -1) then
			if q_Lens ~= nil and f ~= nil then
				...
				Binesh.AddToHTML("p = <span>"..Binesh.Convert_e(f).."*"..Binesh.Convert_e(q_Lens).."/"..Binesh.Convert_e(f).."-"..Binesh.Convert_e(q_Lens).."</span> &#8658; p = "..Binesh.Convert_e(p_Lens).."m")
