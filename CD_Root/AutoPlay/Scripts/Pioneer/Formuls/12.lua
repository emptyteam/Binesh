function Binesh.F12()
	if f == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "›«’·Â ò«‰Ê‰Ì") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "f") ~= -1) then
		if q_Glass ~= nil and p_Glass ~= nil then
			...
			Binesh.AddToHTML("f = <span>"..Binesh.Convert_e(p_Glass).."*"..Binesh.Convert_e(q_Glass).."/"..Binesh.Convert_e(q_Glass).."+"..Binesh.Convert_e(p_Glass).."</span> &#8658; f = "..Binesh.Convert_e(f).." m")
	...
	if q_Glass == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "›«’·Â ”«ÌÂ") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "q") ~= -1) then
		if p_Glass ~= nil and f ~= nil then
			...
			Binesh.AddToHTML("q = <span>"..Binesh.Convert_e(p_Glass).."*"..Binesh.Convert_e(f).."/"..Binesh.Convert_e(p_Glass).."-"..Binesh.Convert_e(f).."</span> &#8658; q = "..Binesh.Convert_e(q_Glass).." m")
	...
	if p_Glass == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "›«’·Â Ã”„") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "p") ~= -1) then
		if q_Glass ~= nil and f ~= nil then
			...
			Binesh.AddToHTML("p = <span>"..Binesh.Convert_e(q_Glass).."*"..Binesh.Convert_e(f).."/"..Binesh.Convert_e(q_Glass).."-"..Binesh.Convert_e(f).."</span> &#8658; p = "..Binesh.Convert_e(p_Glass).." m")
