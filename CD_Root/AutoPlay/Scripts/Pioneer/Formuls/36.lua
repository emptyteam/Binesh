function Binesh.F36()
	if t0 == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "“„«‰") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "t0") ~= -1) then
		if V0_Speed ~= nil and a ~= nil then
			...
			Binesh.AddToHTML("t<sub>0</sub> = <span>"..Binesh.Convert_e(V0_Speed).."/"..Binesh.Convert_e(a).."</span> &#8658; t<sub>0</sub> = "..Binesh.Convert_e(t0).." s")
	...
	if V0_Speed == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "”—⁄ ") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "V0") ~= -1) then
		if t0 ~= nil and a ~= nil then
			...
			Binesh.AddToHTML("V<sub>0</sub> = "..Binesh.Convert_e(t0).."*"..Binesh.Convert_e(a).." &#8658; V<sub>0</sub> = "..Binesh.Convert_e(V0_Speed).." m/s")
			
			Binesh.AddToHTML("a: "..Binesh.Convert_e(a).." m/s<sup>2</sup>" , "V")
			Binesh.AddToHTML("t<sub>0</sub>: "..Binesh.Convert_e(t0).." s", "V")
	...
	if a == nil and ((ListBox.FindItem("Words", -1, LB_BYTEXT, " Êﬁ›") ~= -1 and ListBox.FindItem("Words", -1, LB_BYTEXT, "‘ «»") ~= -1) or (ListBox.FindItem("Words", -1, LB_BYTEXT, "a") ~= -1)) then
		if t0 ~= nil and V0_Speed ~= nil then
			...
			Binesh.AddToHTML("a = <span>"..Binesh.Convert_e(V0_Speed).."/"..Binesh.Convert_e(t0).."</span> &#8658; a = "..Binesh.Convert_e(a).." m/s<sup>2</sup>")
