function Binesh.F43_2()
	if _x1 == nil and ((ListBox.FindItem("Words", -1, LB_BYTEXT, "«Ê·Ì‰ ›«’·Â") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "›«’·Â «Ê·Ì‰") ~= -1) or (ListBox.FindItem("Words", -1, LB_BYTEXT, "x1") ~= -1)) then
		if F ~= nil and k_Coil ~= nil and _x2 ~= nil then
			...
			Binesh.AddToHTML("x<sub>1</sub> = "..Binesh.ProgressNumber(_x2).."+ <span>"..Binesh.ProgressNumber(F).."/"..Binesh.ProgressNumber(k_Coil).."</span> &#8658; x<sub>1</sub> = "..Binesh.ProgressNumber(_x1).." m")
	...
	if _x2 == nil and ((ListBox.FindItem("Words", -1, LB_BYTEXT, "œÊ„Ì‰ ›«’·Â") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "›«’·Â œÊ„Ì‰") ~= -1) or (ListBox.FindItem("Words", -1, LB_BYTEXT, "x2") ~= -1)) then
		if F ~= nil and k_Coil ~= nil and _x2 ~= nil then
			...
			Binesh.AddToHTML("x<sub>2</sub> = "..Binesh.ProgressNumber(_x1).."- <span>"..Binesh.ProgressNumber(F).."/"..Binesh.ProgressNumber(k_Coil).."</span> &#8658; x<sub>2</sub> = "..Binesh.ProgressNumber(_x2).." m")
			
			Binesh.AddToHTML("x<sub>1</sub> = "..Binesh.ProgressNumber(_x2).." m", "V")
			Binesh.AddToHTML("F = "..Binesh.ProgressNumber(F).." N", "V")
			Binesh.AddToHTML("k = "..Binesh.ProgressNumber(k_Coil).." N/m", "V")
