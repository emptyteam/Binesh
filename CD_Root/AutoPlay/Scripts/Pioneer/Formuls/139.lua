function Binesh.F139()
	...
	if k_Self == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "Â” Â") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "k") ~= -1) then
		if L_Self ~= nil and N_Ring ~= nil and A ~= nil and l ~= nil then
			...
			Binesh.AddToHTML("k = <span>"..Binesh.ProgressNumber(l).."*"..Binesh.ProgressNumber(L_Self).."/4*"..Binesh.ProgressNumber(Settings.Zarib.Pi).."*10<sup>-7</sup>*"..Binesh.ProgressNumber(N_Ring).."<sup>2</sup>*"..Binesh.ProgressNumber(A).."</span> &#8658; k = "..Binesh.ProgressNumber(k_Self))
	...
	if N_Ring == nil and ((ListBox.FindItem("Words", -1, LB_BYTEXT, "Õ·ﬁÂ") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "œÊ—") ~= -1) or ListBox.FindItem("Words", -1, LB_BYTEXT, "N") ~= -1) then
		if l ~= nil and L_Self ~= nil and k_Self ~= nil and A ~= nil then
			...
			Binesh.AddToHTML("N = sqrt(<span>"..Binesh.ProgressNumber(l).."*"..Binesh.ProgressNumber(L_Self).."/4*"..Binesh.ProgressNumber(Settings.Zarib.Pi).."*10<sup>-7</sup>*"..Binesh.ProgressNumber(k_Self).."*"..Binesh.ProgressNumber(A).."</span>) &#8658; N = "..Binesh.ProgressNumber(N_Ring))
	...
	if l == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "ÿÊ·") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "l") ~= -1) then
		if k_Self ~= nil and N_Ring ~= nil and L_Self ~= nil and A ~= nil then
			...
			Binesh.AddToHTML("l = <span>"..Binesh.ProgressNumber(k_Self).."*4*"..Binesh.ProgressNumber(Settings.Zarib.Pi).."*10<sup>-7</sup>*"..Binesh.ProgressNumber(N_Ring).."<sup>2</sup>*"..Binesh.ProgressNumber(A).."/"..Binesh.ProgressNumber(L_Self).."</span> &#8658; l = "..Binesh.ProgressNumber(l).." m<sup>2</sup>")
