function Binesh.F18()
	if n_Glass == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "÷—Ì» ‘ò” ") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "n") ~= -1) then
		if V_Speed_Glass ~= nil and C_Speed_Glass ~= nil then
			...
			Binesh.AddToHTML("n = <span>"..Binesh.ProgressNumber( C_Speed_Glass).."/"..Binesh.ProgressNumber(V_Speed_Glass).."</span> &#8658; n = "..Binesh.ProgressNumber(n_Glass))
			
			Binesh.AddToHTML("C = "..Binesh.ProgressNumber(C_Speed_Glass).." m/s", "V")
			Binesh.AddToHTML("V = "..Binesh.ProgressNumber(V_Speed_Glass).." m/s", "V")
	...
	if C_Speed_Glass == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "Œ·«¡ ”—⁄ ") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "C") ~= -1) then
		if V_Speed_Glass ~= nil and n_Glass ~= nil then
			...
			Binesh.AddToHTML("C = "..Binesh.ProgressNumber(n_Glass).."*"..Binesh.ProgressNumber(V_Speed_Glass).." &#8658; C = "..Binesh.ProgressNumber(C_Speed_Glass).."m/s")
	...
	if V_Speed_Glass == nil and (ListBox.FindItem("Words", -1, LB_BYTEXT, "„ÕÌÿ ‘›«› ”—⁄ ") ~= -1 or ListBox.FindItem("Words", -1, LB_BYTEXT, "V") ~= -1) then
		if C_Speed_Glass ~= nil and n_Glass ~= nil then
			...
			Binesh.AddToHTML("V = "..Binesh.ProgressNumber(C_Speed_Glass).."/"..Binesh.ProgressNumber(n_Glass).." &#8658; V = "..Binesh.ProgressNumber(V_Speed_Glass).."m/s")
